...
rule components_unicycler_nano:
    input:
        ...
        path = os.path.join(ASSEMBLY, "{sample}-nanopore-unicycler", "assembly.gfa") # don't actually need this for components.py with unicycler but to ensure script runs
    output:
        out = os.path.join(ASSEMBLY, "{sample}-nanopore-unicycler", "graph_seq_details_unicycler.tsv")
    params:
